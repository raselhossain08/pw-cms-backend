...
export class EnrollmentsService {
  ...
  /**
   * Get download materials for enrollment
   */
  async getDownloadMaterials(
    enrollmentId: string,
    userId: string,
  ): Promise<{
    success: boolean;
    ...
    const enrollment = await this.enrollmentModel
      ...
      .populate('course', 'title materials resources');
    ...
    // Build materials list from course data
    ...
    if (course.resources && Array.isArray(course.resources)) {
      course.resources.forEach((resource: any) => {
        materials.push({
          name: resource.title || resource.name || 'Course Resource',
    ...
    if (course.materials && Array.isArray(course.materials)) {
      course.materials.forEach((material: any) => {
        materials.push({
          name: material.title || material.name || 'Course Material',
    ...
    // If no materials, provide placeholder
    if (materials.length === 0) {
      materials.push({
        name: 'Course Overview',
        type: 'pdf',
        url: `/api/courses/${course._id}/overview`,
        size: 'N/A',
      });
    }
  ...
  async resendWelcomeEmail(enrollmentId: string): Promise<{
    ...
    console.log('[EnrollmentsService] Sending welcome email:', {
      to: student.email,
      studentName: `${student.firstName} ${student.lastName}`,
      courseName: course.title,
    ...
    enrollment.notes.push(
      `Welcome email resent - ${new Date().toISOString()}`,
    );
    await enrollment.save();
  ...
  async extendAccess(
    ...
    enrollment.notes.push(
      `Access extended by ${days} days${reason ? `: ${reason}` : ''} - ${new Date().toISOString()}`,
    );
  ...
  async resetProgress(
    ...
    enrollment.completedLessons.clear();
    enrollment.lessonProgress.clear();
    enrollment.lastAccessedLessons.clear();
    enrollment.totalTimeSpent = 0;
    enrollment.quizzesPassed = 0;
    enrollment.assignmentsCompleted = 0;
    enrollment.completedAt = undefined;
    enrollment.status = EnrollmentStatus.ACTIVE;

    enrollment.notes.push(
      `Progress reset${reason ? `: ${reason}` : ''} - ${new Date().toISOString()}`,
    );
  ...
  /**
   * Get available courses for user (courses not yet enrolled in)
   */
  async getAvailableCourses(
    ...
    if (filters?.category) {
      courseFilter.category = filters.category;
    }

    if (filters?.level) {
      courseFilter.level = filters.level;
    }

    if (filters?.isFree !== undefined) {
      courseFilter.isFree = filters.isFree;
    }
    ...
    const [courses, total, totalPublished] = await Promise.all([
      this.courseModel
        .find(courseFilter)
        .populate('instructor', 'firstName lastName avatar')
        .populate('category', 'name')
        .select(
          'title slug description thumbnail price originalPrice isFree rating reviewCount studentCount level duration totalLessons type status categories tags',
        )
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
        .exec(),
      this.courseModel.countDocuments(courseFilter),
    ...
    return {
      ...
      available: totalPublished - enrolledCourseIds.length,
    };
  }
}
