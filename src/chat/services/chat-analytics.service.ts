...
export class ChatAnalyticsService {
  ...
  private async getConversationStats(dateRange: {
    startDate: Date;
    endDate: Date;
  }) {
    ...
    return {
      ...
      resolutionRate:
        result.total > 0 ? (result.resolved / result.total) * 100 : 0,
  ...
  private async getMessageStats(dateRange: { startDate: Date; endDate: Date }) {
    ...
    return (
      stats[0] || { total: 0, userMessages: 0, agentMessages: 0, aiMessages: 0 }
    );
  }

  private async getResponseTimeStats(dateRange: {
    startDate: Date;
    endDate: Date;
  }) {
  ...
  private async getUserActivityStats(dateRange: {
    startDate: Date;
    endDate: Date;
  }) {
  ...
  private async getStatusDistribution(dateRange: {
    startDate: Date;
    endDate: Date;
  }) {
  ...
  private async getTypeDistribution(dateRange: {
    startDate: Date;
    endDate: Date;
  }) {
  ...
  private async getUserSatisfaction(dateRange: {
    startDate: Date;
    endDate: Date;
  }) {
    // This would integrate with your rating system once implemented
    const satisfactionData =
      await this.chatLoggerService.getSatisfactionMetrics(dateRange);
  ...
  async getRealTimeMetrics() {
    return {
      activeConversations: await this.conversationModel.countDocuments({
        status: 'open',
      }),
      waitingUsers: await this.conversationModel.countDocuments({
        status: 'pending',
      }),
