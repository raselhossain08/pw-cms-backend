...
export class OverviewController {
  ...
  @ApiResponse({ status: 200, description: 'CMS statistics retrieved successfully' })
  async getStats() {
  ...
  @ApiResponse({ status: 200, description: 'CMS sections retrieved successfully' })
  async getSections() {
  ...
  async exportData(@Query('format') format: 'json' | 'csv' = 'json', @Res() res: Response) {
    const result = await this.overviewService.exportData(format);

    if (format === 'csv') {
      res.setHeader('Content-Type', 'text/csv');
      res.setHeader('Content-Disposition', `attachment; filename="cms-overview_${new Date().toISOString().split('T')[0]}.csv"`);
      return res.send(result.data);
    }

    res.setHeader('Content-Type', 'application/json');
    res.setHeader('Content-Disposition', `attachment; filename="cms-overview_${new Date().toISOString().split('T')[0]}.json"`);
