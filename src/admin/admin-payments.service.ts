...
export class AdminPaymentsService {
    ...
    async getPayoutDetails(instructorId: string) {
        ...
        const pendingEarnings = await this.calculateInstructorEarnings(instructorId);
        ...
        return {
            instructor: await this.userModel.findById(instructorId).select('name email avatar'),
            pendingEarnings,
     ...
     */
    async processInstructorPayout(instructorId: string, payoutDto: ProcessPayoutDto) {
        ...
        const availableEarnings = await this.calculateInstructorEarnings(instructorId);
    ...
    async getPaymentAnalytics(filters: AnalyticsFiltersDto) {
        ...
        const totalPayments = overviewData.successfulPayments + overviewData.refundedPayments;
        overviewData['refundRate'] =
            totalPayments > 0 ? (overviewData.refundedPayments / totalPayments) * 100 : 0;
     ...
     * Export transactions
     */
    async exportTransactions(filters: TransactionFiltersDto, format: ExportFormat) {
