...
export class PayPalProvider implements PaymentProvider, OnModuleInit {
  ...
  private async getClient(): Promise<paypal.core.PayPalHttpClient> {
    try {
      ...
      const clientId = integration?.credentials?.clientId || this.configService.get('PAYPAL_CLIENT_ID');
      const clientSecret = integration?.credentials?.clientSecret || this.configService.get('PAYPAL_CLIENT_SECRET');
      const mode = integration?.credentials?.mode || this.configService.get('PAYPAL_MODE') || 'sandbox';
  ...
  async createOrder(amount: number, currency: string, items: any[] = []) {
    ...
    if (!frontendUrl) {
      ...
      throw new BadRequestException('System configuration error: FRONTEND_URL missing');
    }
    ...
    try {
      ...
      this.logger.log(`PayPal CreateOrder - Amount: ${amountFixed}, ItemTotal: ${itemTotalFixed}, Items: ${items.length}`);
      ...
      if (!approveLink) {
        this.logger.warn(`PayPal Order Created but 'approve' link missing. Order ID: ${result.id}. Status: ${result.status}. Links: ${JSON.stringify(links)}`);
      }
      ...
      this.logger.error(`PayPal create order failed: ${error.message}`, error.stack);
      throw new BadRequestException(`PayPal create order failed: ${error.message}`);
    }
  }
  ...
  async createSubscription(planId: string, subscriber: any) {
    ...
    throw new BadRequestException('PayPal subscriptions not fully implemented yet');
  }
