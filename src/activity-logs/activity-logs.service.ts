...
export class ActivityLogsService {
  ...
  async getActivityStats() {
    ...
    const [todayCount, yesterdayCount, totalErrors, criticalErrors] =
      await Promise.all([
  ...
  async getAiStats() {
    ...
    const [totalInteractions, avgResponseTime, byModel, totalTokens] =
      await Promise.all([
  ...
  async getChatStats() {
    ...
    const [totalChats, activeConversations, unreadMessages] = await Promise.all(
      [
        ...
        this.chatLogModel.distinct('conversationId', {
          createdAt: { $gte: today },
        }),
  ...
  async getSystemLogs(query: QuerySystemLogsDto) {
    ...
    if (requiresAction !== undefined)
      filter.requiresAction = requiresAction === 'true';
    if (startDate || endDate) {
      filter.createdAt = {};
      if (startDate) filter.createdAt.$gte = new Date(startDate);
      if (endDate) filter.createdAt.$lte = new Date(endDate);
  ...
  async getSystemStats() {
    const [healthyCount, warningCount, errorCount, criticalCount] =
      await Promise.all([
  ...
  async getLogById(type: string, id: string) {
    ...
    switch (type) {
      case 'activity':
        log = await this.activityLogModel
          .findById(id)
          .populate('userId', 'firstName lastName email')
          .lean()
          .exec();
  ...
  async bulkMarkErrorsAsResolved(
    ids: string[],
    resolvedBy: string,
    solution?: string,
  ) {
