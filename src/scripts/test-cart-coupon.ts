...
async function testCartCoupon() {
    ...
    try {
        ...
        const testCodes = ['TEST_MIN_200', 'TEST_EXPIRED', 'TEST_MAX_USES', 'TEST_VALID_10'];
        ...
        // Add item
        await wishlistService.addToCart(userId, (course as any)._id.toString(), 'Course', TEST_PRICE);

        // Verify Cart Total
        let cart = await wishlistService.getCart(userId);
        console.log(`   Cart Subtotal: $${cart?.items[0].price} * ${cart?.items[0].quantity} = $${cart?.totalAmount}`);
        ...
        if (cart.discount === 10) { // 10% of 100 is 10
            console.log(`✅ Success: Applied 10% off. Discount: $${cart.discount}, Total: $${cart.totalAmount}`);
        } else {
